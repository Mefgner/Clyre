    ...
    async def send_message(user_id: str, message: str, thread_id: str | None = None) -> tuple[str, str]:
        ...
        async with sm.get_session_context_manager() as session:
            if not thread_id:
                ...
                last_order = await get_last_message_order_in_thread(session, thread_id=thread_id, user_id=user_id)

            new_message = await create_message(
                session, user_id=user_id, thread_id=thread_id, role="user", content=message, order=last_order + 1
            )
