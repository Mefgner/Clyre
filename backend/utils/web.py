...
def extract_access_token(token: Annotated[HTTPAuthorizationCredentials, Depends(extract_credentials)]):
    ...
    try:
        ...
        return TokenPayload(**token_dict)
    except ValueError:
...
def extract_service_token(token: Annotated[HTTPAuthorizationCredentials, Depends(extract_credentials)]) -> None:
